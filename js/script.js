...
const CONFIG = {
  ...
  FUNCTIONS_REGION: 'us-central1'            // зміни, якщо деплоїв в інший регіон
};
...
// важливо: той самий регіон, що й у функцій
...
// локальний кеш підписок — фолдбек, якщо читання subscribers недоступне
...
/* ===== Messaging (стабільний потік з модалкою) ===== */
let messaging = null;
...
async function ensureMessagingReady() {
  ...
  // якщо заблоковано — одразу помилка; якщо "default" — дозвіл запитаємо з модалки, тому тут не чіпаємо
...
async function doToggleSubscription(boardId, column, wantSub, btnEl) {
  ...
  try {
    ...
    // локальний кеш стану — щоб зберігався під час ререндерів/перезавантажень
...
async function toggleSubscription(boardId, column, wantSub, btnEl) {
  // Якщо пуші підтримуються, але дозвіл ще не дано — спочатку модалка
...
/* модалка: "Надати дозвіл" (одноразовий хендлер) */
if (notifyTryBtn) {
...
function renderTask(task, listEl, boardId) {
  ...
  el.innerHTML = `
    ...
    <div class="task-url" draggable="false" style="user-select:none;">
      URL: <a href="${escapeAttr(task.url)}" target="_blank" rel="noopener" class="task-link" draggable="false" style="user-select:none;">
        ${escapeHtml(short)}
      </a>
    </div>`;
...
async function initBellsForBoard(wrap, boardId) {
  ...
  try {
    ...
    console.warn('[FCM] getMyTopics failed, fallback to localStorage', e);
  }
